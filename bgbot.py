...
BOT_PREFIX = str(os.getenv('BOT_BG_PREFIX'))
...
async def on_ready():
    ...
    embed = discord.Embed(title="BG Bot Online", color=0x272c88)
    embed.add_field(name=f"**BG Bot came online at {current_time} {current_date}**", value=f"Reporting for ~~strike~~ **duty**, Sir!", inline=True)
...
def load_cogs():
    ...
    for filename in os.listdir('./bgcogs'):
        if filename.endswith('.py') and not "__" in filename:
            client.load_extension(f"bgcogs.{filename[:-3]}")
...
async def load(ctx, cogname):
    if ctx.author.id == OwnerID:
        if f"{cogname}.py" in os.listdir("./bgcogs") and not "__" in cogname:
            try:
                client.load_extension(f"bgcogs.{cogname}")
...
async def unload(ctx, cogname):
    if ctx.author.id == OwnerID:
        if f"{cogname}.py" in os.listdir("./bgcogs") and not "__" in cogname:
            try:
                client.unload_extension(f"bgcogs.{cogname}")
